...
class Transaction:
    ...
    async def insert_transaction(self, user_id, mcc, transaction):
        ...
        try:
            return await self._postgres.execute(INSERT_TRANSACTION, *query_args)
        except exceptions.PostgresError as err:
            LOGGER.error("Could not insert transaction for user=%s: %s. Error: %s", user_id, transaction, err)
